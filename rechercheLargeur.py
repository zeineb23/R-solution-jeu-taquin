...
def update(i) :
    def ajoutDansOPEN(TD,pos) :
        ...
        for j in TD :   
            tab1=[];
            for x in range(0,len(OPEN[i])):
                tab1.append(OPEN[i][x]);
    ...
    global CLOSED;
    pos1=zero(OPEN[i]);
    TD = DeplacementsPossibles(OPEN[i],pos1);
...
affiche(OPEN[0]);
CLOSED=[];
print("Le resultat final en recherche en largeur est : ", RechercheLargeur(T1));
